$(function () {
    ...
    $('.faq__nav-tabs').on('click', 'li:not(.faq__nav-tab-item--active)', function () {
        $(this)
            .addClass('faq__nav-tab-item--active').siblings().removeClass('faq__nav-tab-item--active')
            .closest('div.faq__tabs').find('div.faq__tab-content').removeClass('faq__tab-content--active').eq($(this).index()).addClass('faq__tab-content--active');
    });
    ...
    function singleLetters($words) {
        $words.each(function () {
            ...
            for (i in letters) {
                if (word.parents('.rotate-2').length > 0) letters[i] = '<strong>' + letters[i] + '</strong>';
